...
const AdminDashboard = () => {
  ...
  const fetchPanicAlerts = async () => {
    try {
      ...
      // Fallback to mock data if API fails in development
      if (process.env.NODE_ENV === 'development') {
        console.log('Using fallback mock data for panic alerts');
        setPanicAlerts([
  ...
  const handleResolvePanic = async (alertId) => {
    ...
    try {
      ...
      setToastMessage('Panic alert resolved successfully');
      setToastType('success');
      setShowToast(true);
      
      // Update localStorage if it exists (for offline support)
      const storedAlerts = localStorage.getItem('panicAlerts');
      if (storedAlerts) {
        const alerts = JSON.parse(storedAlerts);
